...
async function resolveLocation(config: WeatherConfig): Promise<ResolvedLocation> {
	...
	if (config.location) {
		...
		return {
			latitude: lat,
			longitude: lon,
			name: admin1 ? `${cityName}, ${admin1}` : cityName,
		};
	}
	...
	create(ctx, config) {
		...
		const tick = async (): Promise<void> => {
			...
			try {
				...
				ctx.logger.warn(
					`forecast fetch failed (${describeError(err)}); keeping last value`,
				);
		...
		void (async () => {
			while (!disposed && !location) {
				try {
					...
					ctx.logger.warn(
						`could not resolve location (${describeError(err)}); retrying in 30s`,
					);
					publishStatus({ ok: false, error: describeError(err) });
					await sleep(30_000);
				}
