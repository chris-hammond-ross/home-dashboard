...
export const demoIntegration = defineIntegration({
	...
	create(ctx, config) {
		...
		const publishEnergy = () => {
			...
			const batteryW =
				clamp(surplus, -5000, 5000) * (batteryPct > 5 && batteryPct < 98 ? 0.9 : 0);
		...
		const DEMO_STATS: Record<string, { name: string; unit: string; unitClass: string }> = {
			"sensor.demo_solar_energy": {
				name: "Demo solar energy",
				unit: "kWh",
				unitClass: "energy",
			},
			"sensor.demo_grid_import": {
				name: "Demo grid import",
				unit: "kWh",
				unitClass: "energy",
			},
			"sensor.demo_grid_export": {
				name: "Demo grid export",
				unit: "kWh",
				unitClass: "energy",
			},
		...
		ctx.registerAction("statistics", (params) => {
			...
			for (const id of statisticIds) {
				...
				for (let ms = Math.floor(start / HOUR_MS) * HOUR_MS; ms < end; ms += HOUR_MS) {
					...
					points.push(
						wantsMean ? { start: ms, mean: kwh * 1000 } : { start: ms, change: kwh },
					);
				}
				out[id] = points;
			}
		...
		ctx.registerAction("call-service", (params) => {
			...
			const targetId = Array.isArray(target.entity_id)
				? target.entity_id[0]
				: target.entity_id;
