  ...
  private async sample(
    ...
    for (const [role, source] of roles) {
      ...
      for (const point of series) {
        ...
        const raw =
          source.kind === "energy" ? (point.change ?? 0) : ((point.mean ?? 0) as number) / 1000;
    ...
    return {
      ...
      missing: roles.map(([role]) => role).filter((role) => !seen.has(role) && role !== "home"),
    };
  }
    ...
    return [...groups.values()]
      ...
      .map((group) => {
        ...
        for (const hour of group.hours) {
          ...
          if (cost.blockId) {
            centsByBand[cost.blockId] = (centsByBand[cost.blockId] ?? 0) + cost.importCents;
          }
