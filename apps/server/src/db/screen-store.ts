...
export class ScreenStore {
	...
	list(): ScreenConfig[] {
		const rows = this.db
			.prepare("SELECT * FROM screens ORDER BY position")
			.all() as ScreenRow[];
	...
	reorder(ids: string[]): void {
		this.db.transaction(() => {
			const existing = (
				this.db.prepare("SELECT id FROM screens").all() as { id: string }[]
			).map((r) => r.id);
	...
	replaceGenerated(screens: ScreenConfig[]): {
		...
		this.db.transaction(() => {
			...
			const survivors = (
				this.db.prepare("SELECT id FROM screens ORDER BY position").all() as {
					id: string;
				}[]
			).map((r) => r.id);
			const setPosition = this.db.prepare("UPDATE screens SET position = ? WHERE id = ?");
