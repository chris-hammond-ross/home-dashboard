			...
			<div className="lw-root">
				<div className="lw-head">
					...
					<button
						...
						className={`lw-roundbtn lw-power${
							onLights.length === 0
								? ""
								: onLights.length === available.length
									? " on"
									: " mixed"
						}`}
						...
						onClick={() =>
							powerEntities(integration, availableIds, onLights.length === 0)
						}
					>
						<IconPower />
					</button>
				...
				{dimmableIds.length ? (
					<HBar
						...
						onSet={(pct) =>
							lightTurnOn(integration, dimmableIds, { brightness_pct: pct })
						}
					/>
				) : null}
					...
					{lights.map((l) => (
						<span
							...
							style={
								l.on && !l.unavailable
									? {
											background: hsl(l.colour),
											boxShadow: `0 0 8px ${hsl(l.colour, 0.8)}`,
										}
									: undefined
							}
						/>
						...
						<div className="lw-grid">
							{lights.map((light) => (
								<LightTile
									...
									light={light}
									onToggle={() =>
										void socket.action(integration, "toggle", {
											entity_id: light.id,
										})
									}
									onSetBrightness={(pct) =>
										lightTurnOn(integration, [light.id], {
											brightness_pct: pct,
										})
									}
								/>
	...
	return (
		...
		>
			...
			>
				...
				>
					{light.unavailable && !light.waiting ? (
						<IconLock />
					) : (
						<LightGlyph icon={light.icon} />
					)}
				</span>
