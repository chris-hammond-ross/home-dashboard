...
export function RadialDial({
  ...
  const valueFrom = (clientX: number, clientY: number) => {
    ...
    return snapTo(view.min + (rel / 270) * (view.max - view.min), view.min, view.max, view.step);
  };
...
export function VFader({
  ...
  return (
    ...
    >
      {ticks
        ? [...Array(9)].map((_, i) => (
            <div key={i} className="cc-fader-tick" style={{ top: `${(i / 8) * 100}%` }} />
          ))
        : null}
      <div
    ...
    >
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <circle
          ...
          style={{
            transition: drag ? "none" : "stroke-dashoffset .3s cubic-bezier(.2,.8,.2,1)",
            filter: `drop-shadow(0 0 6px ${style.glow})`,
          }}
        />
      </svg>
  ...
  return (
    <div className={`cc-mini${glassy ? " is-glassy" : ""}`}>
      ...
      <div className="cc-mini-right">
        ...
        <RoundBtn icon="minus" size={36} onClick={() => onStep(-1)} disabled={view.unavailable} />
        <RoundBtn icon="plus" size={36} onClick={() => onStep(1)} disabled={view.unavailable} />
      </div>
    </div>
  );
}
    ...
    <div className="cc-zone-row">
      ...
      <div className="cc-zone-main">
        <div className="cc-zone-head">
          ...
          <span className="cc-zone-val" style={closed ? undefined : { color: style.accent }}>
            {zone.hasLevel ? (closed ? "Closed" : `${d.shown}%`) : closed ? "Off" : "On"}
          </span>
        </div>
        {zone.hasLevel ? (
          <HSlider
  ...
  return (
    <div className="cc-ztile">
      {zone.hasLevel ? (
        ...
        <div className="cc-ztile-onoff" style={closed ? undefined : { color: style.accent }}>
