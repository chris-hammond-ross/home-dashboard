			...
			<Stack gap="md">
				...
				{!configured ? (
					<Alert color="yellow" variant="light">
						No energy sources are configured for this widget. Add power or energy
						entities in Settings → the power-flow widget, then reopen this panel.
					</Alert>
				) : null}
				...
				{data && totals ? (
					<>
							...
							{node === "grid" ? (
								<StatTile
									...
									sub={
										data.cost ? (
											<SubLine>
												{formatCents(data.cost.exportCents)} credit
											</SubLine>
										) : undefined
									}
								/>
								...
								<StatTile
									...
									sub={
										totals.kwh.solar > 0 ? (
											<SubLine>
												{Math.round(
													(totals.mix.solar / totals.kwh.solar) * 100,
												)}
												% of generation
											</SubLine>
										) : undefined
									...
									<StatTile
										...
										sub={
											costDelta === null ? (
												...
												<SubLine>
													{costDelta >= 0 ? "↑" : "↓"}{" "}
													{Math.abs(costDelta).toFixed(0)}% vs{" "}
													{range?.previousLabel}
												</SubLine>
											)
										}
										...
										>
											<Group gap={8} wrap="nowrap">
												...
												<Text size="xs" c="var(--text-muted)">
													{band.centsPerKwh.toFixed(2)}c/kWh ·{" "}
													{kwh(band.kwh)}
												</Text>
											</Group>
											<Text
						...
						{!data.cost ? (
							<Text size="xs" c="var(--text-muted)">
								No tariff is active — add one in Settings → Energy tariff to see
								costs here.
							</Text>
						) : null}
						...
						{data.estimated ? (
							<Text size="xs" c="var(--text-muted)">
								Estimated: some figures are integrated from power sensors rather
								than metered energy. Set the kWh statistics in the widget’s settings
								for exact numbers.
							</Text>
						) : null}
						...
						{data.missing.length ? (
							<Text size="xs" c="var(--text-muted)">
								No history for: {data.missing.join(", ")}. Home Assistant may not
								have recorded these long enough yet.
							</Text>
						) : null}
                ...
                each end. Same house, same zone normally — say so when not. */}
						{data.tariff && data.tariff.timezone !== browserZone ? (
							<Text size="xs" c="var(--status-warning)">
								Your tariff is set to {data.tariff.timezone} but this device is in{" "}
								{browserZone}. Day boundaries will be slightly off — set them to
								match in Settings → Energy tariff.
							</Text>
						) : null}
					</>
				) : null}
