...
function DayRibbon({
  ...
  const ramp = useMemo(() => {
    ...
    return new Map(sorted.map((b, i) => [b.id, BAND_COLORS[Math.min(i, BAND_COLORS.length - 1)]!]));
  }, [blocks]);
      ...
      <Stack gap="xs">
        <Group gap="xs" wrap="nowrap" align="flex-end">
          ...
          <Select
            ...
            value={block.kind}
            onChange={(value) => value && onChange({ ...block, kind: value as RateKind })}
            allowDeselect={false}
          />
          <NumberInput
            size="xs"
            ...
            onChange={(value) =>
              onChange({ ...block, centsPerKwh: typeof value === "number" ? value : 0 })
            }
          />
          <ActionIcon
            size="lg"
              ...
              {ALL_DAYS.map((day) => (
                <Checkbox
                  ...
                  onChange={(e) => {
                    ...
                    if (next.size) setWindow({ days: ALL_DAYS.filter((d) => next.has(d)) });
                  }}
                />
              ))}
            </Group>
      ...
      return {
        ...
        [which]: [
          ...d[which],
          {
            ...
            centsPerKwh: 0,
            windows:
              which === "importBlocks" ? [{ days: ALL_DAYS, startMin: 960, endMin: 1260 }] : [],
          },
        ],
      };
    });
      ...
      {!draft && tariffs ? (
        <Stack gap="xs">
          {tariffs.length === 0 ? (
            <Card padding="lg">
              <Text size="sm" c="var(--text-muted)">
                No tariff yet. Import your plan’s published rates, or create one and enter them by
                hand — either way the power-flow widget starts showing cost.
              </Text>
            </Card>
          ) : null}
          {tariffs.map((tariff) => (
              ...
              <Group justify="space-between" wrap="nowrap">
                <div style={{ minWidth: 0 }}>
                  ...
                  <Text size="xs" c="var(--text-muted)">
                    ...
                    {tariff.dailySupplyCents.toFixed(1)}c/day supply · {tariff.timezone}
                    {tariff.source ? ` · imported from ${tariff.source.brandName ?? "CDR"}` : ""}
                  </Text>
                </div>
                <Group gap="xs" wrap="nowrap">
                  {!tariff.active ? (
                    <Button
                      size="xs"
                      variant="light"
                      onClick={() => void setActiveTariff(tariff.id).then(reload)}
                    >
                      Make active
                    </Button>
                  ) : null}
        ...
        <Stack gap="md">
          <Group gap="sm" align="flex-end">
            ...
            <TextInput
              ...
              onChange={(e) => setDraft({ ...draft, timezone: e.currentTarget.value })}
            />
            <NumberInput
              size="sm"
              w={170}
              ...
              onChange={(value) =>
                setDraft({ ...draft, dailySupplyCents: typeof value === "number" ? value : 0 })
              }
            />
          </Group>
          ...
          <Stack gap={4}>
            ...
            <Text size="xs" tt="uppercase" fw={600} lts="0.08em" c="var(--text-muted)" mt="xs">
              Saturday
            </Text>
            <DayRibbon blocks={draft.importBlocks} timezone={draft.timezone} day={5} />
            {draft.importBlocks.every((b) => b.windows.length > 0) ? (
              <Text size="xs" c="var(--text-muted)">
                Grey means no band covers that hour — add a catch-all so every hour is priced.
              </Text>
            ) : null}
          </Stack>
          ...
          <Stack gap="xs">
            <Group justify="space-between">
              ...
              <Button size="compact-xs" variant="default" onClick={() => addBlock("importBlocks")}>
                Add band
              </Button>
            </Group>
          ...
          <Stack gap="xs">
            <Group justify="space-between">
              ...
              <Button size="compact-xs" variant="default" onClick={() => addBlock("exportBlocks")}>
                Add band
              </Button>
            </Group>
