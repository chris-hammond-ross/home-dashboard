			...
			switch (field.kind) {
				...
				case "number":
					return (
						<NumberInput
							...
							description={field.help}
							value={
								typeof props[field.key] === "number"
									? (props[field.key] as number)
									: ""
							}
							onChange={(value) =>
								setProp(field.key, typeof value === "number" ? value : null)
							}
						/>
					);
				case "boolean":
					return (
						<Switch
							...
							onChange={(e) =>
								setProp(field.key, e.currentTarget.checked ? true : null)
							}
						/>
					);
				case "select":
				...
				case "entity-list": {
					const ids = Array.isArray(props[field.key])
						? (props[field.key] as unknown[]).filter(
